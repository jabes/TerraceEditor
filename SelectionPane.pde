private class SelectionPane {
  ...
  void drawAlignmentPad () {
    ...
    int alignPadX = globals.viewportWidth + 10,
        alignPadY = globals.viewportHeight - 49;
    ...
    for (int i = 0; i < alignmentButtonData.length; i++) {
      ...
      if (
        ...
        mouse.cursor = HAND;
        if (mouse.wasClicked) tileAlignment = button[4];
      }
      
      pushStyle();
      if (button[4] == tileAlignment) fill(200, 121, 214);
      else fill(200);
  ...
  void drawButton (int x, int y, int w, int h, PImage gfx, color c) {
    ...
    int tileX,
        tileY = menuContentOffsetTop + buttonSpacing + buttonRowCount * (blocksLayer.tileHeight + buttonSpacing);
  ...
  void drawTab (int i, PImage tabIcon) {
    ...
    pushStyle();
    if (selectedTab == i) fill(255);
    else if (isMouseOver && !dialog.isOpen && !changeMapSizeWindow.isOpen) fill(220); 
    else fill(200);
    ...
    if (selectedTab != i) {
      ...
      pushStyle();
      stroke(100);
      if (i > 0) line(tabX - 1, tabY, tabX - 1, tabY + tabHeight - 1);
      if (i < 2) line(tabX + tabWidth - 1, tabY, tabX + tabWidth - 1, tabY + tabHeight - 1);
      popStyle();
    }
